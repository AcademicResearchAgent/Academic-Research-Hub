...
不用：数据是 numpy `.npy`（走 `npy3d-visualization` 技能）；
...
| 工具 | 作用 |
|---|---|
| `mcp__cfd_npy3d__pvdata_inspect` | 检视文件：reader/包围盒/点数/时间步/数组清单（先检视再画图，避免瞎猜数组名与时间步） |
...
```
mcp__cfd_npy3d__pvdata_inspect(file_path="/path/to/disk_out_ref.ex2")
mcp__cfd_npy3d__pvdata_render_scatter3d(file_path="/path/to/disk_out_ref.ex2", array_name="Temp", timestep_index=0, case="mocvd_reactor")
mcp__cfd_npy3d__pvdata_render_animation(file_path="/path/to/case.pvd", array_name="pressure", max_frames=24, fps=6, case="sim1")
```
...
- 扩展名无法识别：先 `mcp__cfd_npy3d__pvdata_inspect` 看 reader 报错，必要时换用显式 reader 或先转换格式。
- 指定的 `array_name` 不存在：报错并列出可用数组，改按清单里的名字重试。
- `timestep_index` 越界：明确报错，先 inspect 取 `TimestepValues` 长度。
