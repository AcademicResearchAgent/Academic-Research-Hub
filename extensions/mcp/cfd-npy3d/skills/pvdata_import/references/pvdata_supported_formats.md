...
## 权威清单：`config/paraview_readers.json`
...
`pvdata_import` 首先按扩展名选择显式 reader，未命中扩展名时用
`pvs.OpenDataFile` 让 ParaView 自动探测兜底，因此 **reader 清单中的全部格式
均可导入**。

如需在自己机器上重新生成该清单（或升级 ParaView 后刷新）：
...
> 完整、权威的扩展名↔reader 映射以 `config/paraview_readers.json` 为准；
> 上表只用于快速概览。

## 是否"2D 平面场"的判定

`pvdata_import` 逐帧读取数据包围盒，若
...
数据改用 `pvdata_render_scatter3d / pvdata_render_animation`。
