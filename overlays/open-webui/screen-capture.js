...
/**
 * @param {(files: File[]) => unknown} onFiles
 * @param {{info: (message: string) => unknown, error: (message: string) => unknown}} toast
 */
export async function captureScreenshot(onFiles, toast) {
    ...
    try {
        ...
        const blob = await new Promise(/** @param {(blob: Blob | null) => void} resolve */ resolve => canvas.toBlob(resolve, 'image/png'));
