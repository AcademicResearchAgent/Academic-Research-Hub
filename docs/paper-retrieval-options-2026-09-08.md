...
结论：DeepSeek＋Hermes 与 Codex＋GPT 都能发现公开论文、调用学术 API、获取部分全文。Codex 提供现成的网页搜索集成；当前 Hermes 工作站的搜索和多步取文也已实际跑通。支撑 MainTask 的关键建设是可复现的学术检索、可靠全文获取和证据管理。已验证的公开数据接口可以为两种方案共用，不需要因为数据源接入而先更换模型。
