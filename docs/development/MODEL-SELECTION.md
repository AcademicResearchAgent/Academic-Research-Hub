...
保留原来的“科研助手”默认入口，继续使用服务器已有配置。新增具名模型使用个人 Key，不会回退到服务器的默认模型。关闭自动随机选模型的 Arena 入口，避免绕过明确的厂商和密钥选择；原设置备份在服务器 `openwebui/model-catalog-backup/evaluations.json`。

公网上填写 Key 需要 HTTPS。还没有域名时，可在有服务器 SSH 权限的电脑运行以下命令，随后打开 `http://localhost:19119`；通信通过 SSH 加密：
...
Qwen、Kimi、GLM 尚未使用有效付费 Key 完成真实对话验收；会在用户首次填写时向对应厂商验证。不能将目录展示、无效 Key 检查或模拟测试当成这些厂商的完整实测结果。当前公网入口仍为 HTTP，首次填 Key 使用 HTTPS 或 SSH 隧道。
