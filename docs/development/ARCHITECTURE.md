...
生产 UI 为 `haudi-openwebui:0.11.3-research-v3`，镜像 ID 与基线 ID 记录在根目录锁文件。它仍采用编译产物覆盖的旧构建方式。新整理的源码补丁与现有定制意图对应，但尚未以完整源码构建、验收并替换生产镜像；两者不是已经验证为字节一致的产物。

## 已有能力和待建模块

已有：账号登录、科研任务聊天、模型调用、执行引擎工具、科研文案、身份规则，以及论文检索可行性实测。论文检索调研见上一层的调研报告。
